...
class PubSubService:
    ...
    def __init__(self, project_id: str = None, topic_name: str = None):
        ...
        self.topic_name = topic_name or os.getenv("PUBSUB_TOPIC_NAME", "order-created")
    ...
    def publish_order_created_event(self, order_data: Dict[str, Any]) -> bool:
        """
        Publish a order created event to the configured topic.
        ...
        try:
            ...
            print(
                f"Order created event published successfully with message ID: {message_id}"
            )
            return True

        except Exception as e:
            print(f"Error publishing order created event: {str(e)}")
            return False
